...
from cylc.uiserver.graphql.render_graphiql import (
...
class TornadoGraphQLHandler(web.RequestHandler):
    ...
    async def run(self, *args, **kwargs):
        try:
            ...
            if show_graphiql:
                query, variables, operation_name, _ = self.get_graphql_params(
                    self.request, data
                )
                graphiql = await self.render_graphiql(
                    query=query or "",
                    variables=(
                        "" if variables is None else json.dumps(variables)
                    ),
    ...
    async def get_response(self, data, show_graphiql=False):
        ...
        if execution_result:
            ...
            result = self.json_encode(response, pretty=show_graphiql)
        else:
            result = None
    ...
    async def execute_graphql_request(
        self, data, query, variables, operation_name, show_graphiql=False
    ):
        if not query:
            if show_graphiql:
                return None
    ...
    async def render_graphiql(
        ...
        graphiql_data = GraphiQLData(
            ...
            variables=variables,
            operation_name=operation_name,
            subscription_url=self.subscription_path,
        ...
        return await render_graphiql_async(
            data=graphiql_data,
            config=graphiql_config,
            options=graphiql_options
        )

    def can_display_graphiql(self, data):
        raw = (
            "raw" in self.request.query_arguments
            or "raw" in self.request.arguments
            or "raw" in data
        )
        return not raw and self.request_wants_html(self.request)
